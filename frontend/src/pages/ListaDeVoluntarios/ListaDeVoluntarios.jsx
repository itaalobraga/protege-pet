  ...
  const carregarVoluntarios = useCallback(async (termo = "") => {
    ...
    try {
      const endpoint = termo
        ? `/voluntarios?busca=${encodeURIComponent(termo)}`
        : "/voluntarios";
    ...
    <>
      ...
      <main>
        <Container className="py-4">
          <div className="d-flex justify-content-between align-items-center mb-4">
            <h5 className="mb-0 fw-semibold">Voluntários</h5>
                ...
                <Table hover responsive className="text-center mb-0">
                  <thead className="table-light">
                    <tr>
                      ...
                      <th className="fw-semibold">Telefone Celular</th>
                      <th className="fw-semibold">Telefone Residencial</th>
                    ...
                    {voluntarios.length === 0 ? (
                      ...
                      voluntarios.map((voluntario) => (
                        <tr key={voluntario.id}>
                          <td className="align-middle">{voluntario.vlt_nome}</td>
                          <td className="align-middle">{voluntario.vlt_cpf}</td>
                          <td className="align-middle">{voluntario.vlt_telefone}</td>
                          <td className="align-middle">{voluntario.vlt_tel_Residencial}</td>
                          <td className="align-middle">{voluntario.vlt_email}</td>
                          <td className="align-middle">{voluntario.vlt_disponibilidade || "-"}</td>
                          <td className="align-middle">
                            <div className="d-flex gap-2 justify-content-center">
                              <Link
                                to={`/voluntarios/cadastro/editar/${voluntario.id}`}
      ...
      >
        <Modal.Body className="text-center py-4">
          ...
          <p className="text-secondary mb-4">Deseja realmente excluir este voluntário?</p>
